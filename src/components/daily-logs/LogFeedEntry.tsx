...
export function LogFeedEntry({
  ...
  return <article className="space-y-3 py-4 border-b border-border last:border-b-0">
      {/* Header with chips */}
      

      {/* Title */}
      {item.title && <h4 className="font-medium text-foreground leading-relaxed">{item.title}</h4>}

      {/* Content */}
      {item.body && <div className="prose prose-sm max-w-none text-foreground">
          <p className="whitespace-pre-wrap leading-relaxed">{item.body}</p>
        </div>}

      {/* Execution report details */}
      {item.kind === 'execution_report' && <div className="space-y-2">
          {item.reason_label && <div className="text-sm">
              <span className="font-medium text-foreground">Reason: </span>
              <span className="text-muted-foreground">{item.reason_label}</span>
            </div>}
          
          {item.detail && <div className="text-sm">
              <span className="font-medium text-foreground">Detail: </span>
              <span className="text-muted-foreground">{item.detail}</span>
            </div>}

          {item.review_comment && <div className="text-sm">
      ...
      {item.media_count && item.media_count > 0 && <div className="mt-3 p-3 bg-muted/50 rounded-lg text-sm text-muted-foreground">
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground pt-2">
        ...
        {item.kind === 'note' && item.todo_id && <Button variant="outline" size="sm" onClick={() => onFillReport(item.todo_id!)} className="h-6 px-2 text-xs">
            Fill Report
          </Button>}
      </div>
    </article>;
