...
export function LogFeed({
  ...
  return <div className="space-y-6">
      {/* Filters */}
      <LogFeedFilters searchValue={searchValue} onSearchChange={setSearchValue} dateFrom={dateFrom} dateTo={dateTo} onDateFromChange={setDateFrom} onDateToChange={setDateTo} authorFilter={authorFilter} onAuthorFilterChange={setAuthorFilter} typeFilter={typeFilter} onTypeFilterChange={setTypeFilter} projectId={selectedProject.id} />

      {/* Feed */}
      {loading && page === 1 ? <div className="text-center py-12">
          <div className="text-muted-foreground">Loading daily logs...</div>
        </div> : <div className="space-y-8">
          {groupedItems.map(group => <div key={group.date} className="space-y-4">
              {/* Date Header */}
      ...
      {/* Load More */}
      {hasMore && !loading && feedItems.length > 0 && <div className="text-center py-6">
      ...
      {/* Loading More */}
      {loading && page > 1 && <div className="text-center py-6">
      ...
      {!loading && feedItems.length === 0 && <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mb-4">
            {searchValue ? "Try adjusting your search terms or date range." : "Create your first log entry or check if there are any to-dos due in this date range."}
          </p>
      ...
      {/* Modals */}
      <ExecutionReportSheet open={showExecutionReport} onOpenChange={setShowExecutionReport} todoId={selectedTodoId} todoTitle={selectedTodoTitle} onSuccess={handleSuccess} />
